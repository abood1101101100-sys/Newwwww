    ...
    for new_mem in new_members:
        ...
        if should_welc:
            ...
            # Welcome Support
                ...
                update.effective_message.reply_text(
                    "انضم أحد مشرفي الدعم إلى المجموعة!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"انضم مستخدم دعم البوت إلى المجموعة"
                )
                continue
            ...
            # Welcome Whitelisted
                ...
                update.effective_message.reply_text(
                    "انضم مستخدم Tiger إلى المجموعة!", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"انضم مستخدم موثوق إلى المحادثة"
                )
                continue
            ...
            # Welcome Tigers
                ...
                update.effective_message.reply_text(
                    "انضم مستخدم Wolf إلى المجموعة!", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if var in ("no", "off"):
                ...
                update.effective_message.reply_text("خدمة تنظيف الترحيب: مفعّلة")
            else:
                update.effective_message.reply_text(
        ...
        if curr:
            update.effective_message.reply_text(
                "خدمة تنظيف الترحيب: <code>مفعّلة</code>", parse_mode=ParseMode.HTML
            )
        else:
            update.effective_message.reply_text(
                "خدمة تنظيف الترحيب: <code>معطلة</code>", parse_mode=ParseMode.HTML
            )
...
WELC_HELP_TXT = (
    ...
    "رسائل الترحيب تدعم الماركداون، يمكنك استخدام النص العريض/المائل/الكود/الروابط. "
