...
# الأوامر تعمل بـ / أو ! أو بدون أي prefix
CMD_STARTERS = ("/", "!", "")
    ...
    def check_update(self, update):
        if isinstance(update, Update) and update.effective_message:
            ...
            if message.text and len(message.text) > 0:
                ...
                # دعم الأوامر بـ / أو ! أو بدون prefix
                has_prefix = any(fst_word.startswith(start) for start in ("/", "!") if start)
                if has_prefix:
                    args = message.text.split()[1:]
                    command = fst_word[1:].split("@")
                else:
                    args = message.text.split()[1:]
                    command = fst_word.split("@")
                if has_prefix or True:
